...
public class StuController extends LogController{
	...
	public void userList(HttpServletRequest request,Student stu,HttpServletResponse response,String offset,String limit) throws Exception{
		try {
			...
			*/
			...
			PageInfo<Student> userList= ss.findStuPage(stu,pageNum,pageSize,ordername,order);
	...
	public void reserveStu(HttpServletRequest request,Student stu,String[] hobbys,MultipartFile file,User user,HttpServletResponse response){
		...
		try {
			if (sId != null) {   // userId不为空 说明是修改
				...
				if(ss.findByName(stu.getSname())==null){  // 没有重复可以添加
					if (filename!=null && filename!="") {
						String newName = UUID.randomUUID().toString()+filename.substring(filename.lastIndexOf("."));
						stu.setImg("/img/"+newName);
						File file2 = new File("E:/img/"+newName);
